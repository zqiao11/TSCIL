...
class GSSGreedyUpdate(object):
    def __init__(self, params):
        ...
        self.buffer_score = torch.FloatTensor(params.mem_size).fill_(0).to(self.device)
    ...
    def update(self, buffer, x, y, **kwargs):
        ...
        place_left = buffer.buffer_input.size(0) - buffer.current_index
        if place_left <= 0:  # buffer is full
            ...
            if batch_sim < 0:
                ...
                buffer.buffer_input[index[sub_index]] = x[added_indx[sub_index]].clone()
            ...
            buffer.buffer_input[buffer.current_index:buffer.current_index + offset].data.copy_(x)
    ...
    def get_rand_mem_grads(self, buffer, grad_dims):
        ...
        for i in range(num_mem_subs):
            ...
            batch_x = buffer.buffer_input[random_batch_inds]
