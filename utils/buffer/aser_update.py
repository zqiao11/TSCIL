...
class ASER_update(object):
    def __init__(self, params, **kwargs):
        ...
        self.k = params.aser_k
        self.mem_size = params.mem_size
        self.num_tasks = params.num_tasks  # Buffer里初始化了，不同stream不同
        self.out_dim = get_num_classes(params)  # 直接给stream里的N_all cls
        self.n_smp_cls = int(params.aser_n_smp_cls)
        self.n_total_smp = int(params.aser_n_smp_cls * self.out_dim)
    ...
    def _update_by_knn_sv(self, model, buffer, cur_x, cur_y):
        ...
        eval_x, eval_y, eval_indices = \
            ClassBalancedRandomSampling.sample(buffer.buffer_input, buffer.buffer_label, self.n_smp_cls,
                                               device=self.device)
        ...
        ind_cur = sv_arg_large[nonzero_indices(sv_arg_large >= n_cand_buf)] - n_cand_buf  # 是不是一直是空？
        arg_buffer = sv_arg_small[nonzero_indices(sv_arg_small < n_cand_buf)]
        arg_buffer = arg_buffer.to('cpu')
        ...
        buffer.buffer_input[ind_buffer] = x_upt
