...
def random_retrieve(buffer, num_retrieve, excl_indices=None, return_indices=False):
    ...
    x = buffer.buffer_input[indices]
    y = buffer.buffer_label[indices]

    if buffer.buffer_logits is not None:
        logits = buffer.buffer_logits[indices]
        y = (y, logits)

    if buffer.buffer_sub is not None:
        sub_saved = buffer.buffer_sub.cpu().detach().numpy()
        num_sub = len(set(list(sub_saved)))
        num_retrieve_per_sub = int(num_retrieve / num_sub)
        indices = []
        for i in range(num_sub):
            valid_indices_i = np.where(sub_saved==i)[0]
            indices_i = np.random.choice(valid_indices_i, num_retrieve_per_sub, replace=False)
